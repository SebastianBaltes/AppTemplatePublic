(function($){
    $.fn.bootstrapTransfer = function(options) {
        ...
        return this.each(function(){
            ...
            _this.$selector_chosen_hidden_inputs = _this.find('.selector-chosen-hidden-inputs');
            ...
            _this.$target_select.attr('name', settings.target_name);
            /* height */
            ...
            _this.update_lists = function(force_hilite_off) {
                ...
                _this.$selector_chosen_hidden_inputs.empty();
                var index = 0;
                _this.$target_select.find('option').each(function() {
                	var option = $(this);
                	var name = settings.target_name.replace(/#/g,index++);
                	_this.$selector_chosen_hidden_inputs.append('<input type="hidden" name="'+name+'" value="'+option.val()+'" />');
                });
            };
                ...
                <tr>\
                    <td width="50%">\
                        <div class="selector-available">\
                            ...
                            <a href="#" class="selector-chooseall">Alles auswählen</a>\
                        </div>\
                    </td>\
                    <td>\
                        <div class="selector-chooser">\
                            <a href="#" class="selector-add">hinzufügen</a>\
                            <a href="#" class="selector-remove">entfernen</a>\
                        </div>\
                    </td>\
                    <td width="50%">\
			            <div class="selector-chosen-hidden-inputs">\
			        	</div>\
                        <div class="selector-chosen">\
        					<div class="selector-chosen-border">\
	                            <h2>Ausgewählt</h2>\
	                            <div class="selector-filter right">\
	                                <p>auswählen und Pfeil anklicken</p>\
	                            </div>\
        						<select multiple="multiple" class="filtered target">\
        						</select>\
        					</div>\
                            <a href="#" class="selector-clearall">Alles löschen</a>\
                        </div>\
                    </td>\
                </tr>\
            </table>',
